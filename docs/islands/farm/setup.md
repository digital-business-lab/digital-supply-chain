...
> Replace `192.168.10.10` with the DHCP-reserved IP of the workstation (set up in [Workstation Setup § Network](../../operations/setup-workstation.md#4-network-dhcp-reservation)). Port `1700/UDP` is received by the ChirpStack Gateway Bridge.
...
> Grafana opens automatically in kiosk mode on the touch display at boot (configured in [Workstation Setup § Grafana kiosk mode](../../operations/setup-workstation.md#12-grafana-kiosk-mode-touch-display)). No manual browser launch is needed.
...
| Automated GitOps deploy and backup timers | [GitOps Workflow](../../operations/gitops.md) |
