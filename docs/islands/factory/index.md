...
4. **Fabric events + Distributor hand-off**
   ...
   - `receive_factory_shipment` is a custom ERPNext whitelisted method on the Distributor side; it is not part of the ERPNext standard REST API. Its implementation is still conceptual in this repository's current AI workflow phase and should later be documented with the Distributor ERP integration and API contract placeholder in the Distributor documentation once production code is written.
