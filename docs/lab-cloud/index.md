...
Note: brewing parameters are **not** written to the Hyperledger Fabric ledger. They are per-cup operational data, not supply chain events. See [hyperledger-fabric.md](../architecture/hyperledger-fabric.md#what-is-not-on-chain).
...
The Lab Cloud configuration lives in `lab-cloud/` in the repository, structured analogously to the island folders. See [Lab Cloud Setup Guide](setup.md) for the full folder layout and step-by-step instructions.
