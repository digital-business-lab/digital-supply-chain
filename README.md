...
> **Tip (optional):** If you want the `apps/` sources to persist & be shared, add a named volume (e.g. `apps:`) and mount it at `/home/frappe/frappe-bench/apps` on **backend, queue-short, queue-long, scheduler, websocket**. You’ll still `pip install` in each container’s venv.
...
> Tip: for test mode, use your gateway’s sandbox credentials.
